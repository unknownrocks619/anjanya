<div class="row g-2 bg-white">
    <div class="col-md-12 mt-4">
        <div class="card-body">
            <form method="post" action="{{ route('admin.pages.upload', ['page' => $page, 'current_tab' => 'media']) }}"
                data-max-file='6'>
        ...
        @foreach ($content as $image)
            <div class="row mt-3 border-bottom mb-2">
                ...
                <div class="col-md-3 d-flex align-items-center">
                    <h3 class="text-dark">{{ $page->title }} @if ($image->type)
                            - {{ \App\Models\Page::IMAGE_TYPES[$image->type] }}
                        @endif
                    </h3>
                </div>
                <div class="col-md-3 d-flex align-items-center">
                    <select name="image_type[]" class="form-control update-from-select"
                        data-action="{{ route('admin.pages.update_image_type', ['page' => $page, 'image_relation' => $image, 'current_tab' => 'media']) }}">
                        <option value="">Select Option</option>
                        @foreach (\App\Models\Page::IMAGE_TYPES as $key => $type)
                ...
                <div class="col-md-3 d-flex justify-content-end">
                    <button type='button' data-confirm='Are you Sure ?'
                        data-action="{{ route('admin.pages.remove_image', ['page' => $page, 'image_relation' => $image, 'current_tab' => 'media']) }}"
