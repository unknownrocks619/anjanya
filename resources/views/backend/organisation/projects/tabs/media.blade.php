    ...
    <div class="col-md-12 mt-4">
        <div class="card-body">
            <div class="dropzone dropzone-info dz-area" id="fileTypeValidation"
                action="{{ route('admin.org.projects.upload', ['project' => $project, 'current_tab' => 'media', 'org' => $org]) }}"
                data-max-file='6'>
        ...
        @foreach ($content as $image)
            <div class="row mt-3 border-bottom mb-2">
                ...
                <div class="col-md-3 d-flex align-items-center">
                    <h3 class="text-dark">{{ $project->title }} - {{ $image->type }}</h3>
                </div>
                <div class="col-md-3 d-flex align-items-center">
                    <select name="image_type[]" class="form-control update-from-select"
                        data-action="{{ route('admin.org.projects.update_image_type', ['project' => $project, 'image_relation' => $image->getKey(), 'current_tab' => 'media']) }}">
                        <option value="">Select Option</option>
                        @foreach (\App\Models\Project::IMAGE_TYPE as $key => $type)
                ...
                <div class="col-md-3 d-flex justify-content-end">
                    <button type='button' data-confirm='Are you Sure ?' data-method='post'
                        data-action="{{ route('admin.org.projects.remove_image', ['project' => $project, 'org' => $org, 'image_relation' => $image, 'current_tab' => 'media']) }}"
