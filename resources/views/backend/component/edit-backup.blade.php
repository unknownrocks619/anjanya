                            ...
                            <div class="card">
                                <div class="card-body" style="min-height:80vh;max-height:80vh; overflow:scroll">
                                    <ul data-method="post" class="component-parent datatable-sortable-component"
                                        style="min-height:80vh;max-height:80vh; overflow:scroll"
                                        data-action="{{ route('admin.sort.re-order-column', ['model_name' => 'ComponentBuilder']) }}">
                                        @foreach ($webcomponent->getComponents as $component)
                                            <li data-sort-id="{{ $component->getKey() }}" class="common-component-selector"
                                                data-component-name="{{ $component->component_type }}">
                                                <div
                                                    class="d-flex justify-content-between btn btn-primary align-items-center">
                                                    <div class="">
                                                        <i class="fa fa-ellipsis-v fs-4 me-3 sortable-handle"
                                                            style="cursor:grab"></i>
                                                        <a href="#" onclick="CB.selectComponent(this)"
                                                            class="text-white"
                                                            data-url="{{ route('admin.components.common.edit', ['webcomponent' => $webcomponent, 'component_name' => $component->component_type, 'componentID' => $component->getKey()]) }}">
                                                            {{ $component->component_name }}</a>
                                                    </div>
                                                    <div class="text-end">
                                                        <a href="#" onclick="CB.removeComponent(this)"
                                                            data-component-name="{{ $component->component_type }}"
                                                            data-component-id="{{ $component->getKey() }}"><i
                                                                class="fa fa-trash text-danger fs-5"></i></a>
                                                    </div>
                                                </div>
                                            </li>
                                        @endforeach
                            ...
                            <div class="row">
                                <div class="col-md-12 text-end">
                                    <button type="button" class="btn btn-primary action-button"
                                        onclick="window.CB.updateComponent()" disabled>
                                        Update Component
                                    </button>
                                </div>
                            </div>
