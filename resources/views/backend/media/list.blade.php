    ...
    <div class="row g-2 bg-white">
        <div class="col-md-12 mt-4">
            <div class="card-body">
                <form method="post"
                    action="{{ route('admin.media.upload', ['model' => $model::class, 'model_id' => $model->getKey()]) }}"
                    data-max-file='6'>
        ...
        <div class="col-md-12 mt-3 bg-light">
            ...
            @foreach ($content as $image)
                <div class="row mt-3 border-bottom mb-2">
                    <div class="col-md-3">
                        @if (strtoupper(pathinfo($image->image->original_filename, PATHINFO_EXTENSION)) == 'PDF')
                            <svg height="64px" width="64px" version="1.1" id="Layer_1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                viewBox="0 0 303.188 303.188" xml:space="preserve" fill="#000000">
                                ...
                                <g id="SVGRepo_iconCarrier">
                                    <g>
                                        ...
                                        <polygon style="fill:#FB3449;"
                                            points="227.64,25.263 32.842,25.263 32.842,0 219.821,0 "></polygon>
                                        <g>
                                        ...
                                        <polygon style="fill:#D1D3D3;"
                                            points="219.821,50.525 270.346,50.525 219.821,0 "></polygon>
                                    </g>
                                </g>
                            </svg>
                        @else
                    ...
                    <div class="col-md-3 d-flex align-items-center">
                        <h3 class="text-dark">
                            @if ($image->type)
                                - {{ $model::IMAGE_TYPES[$image->type] ?? 'Associates File' }}
                            @endif
                        </h3>
                    </div>
                    ...
                    <div class="col-md-3 d-flex align-items-center">
                        <select name="image_type[]" class="form-control update-from-select"
                            ...
                            @foreach ($model::IMAGE_TYPES as $key => $type)
                        ...
                        <input type="text" value="{{ $image->title }}" name="title"
                            class="form-control update-from-select ms-2"
                            data-action="{{ route('admin.media.update_image_type', ['image_relation' => $image, 'model' => $model::class, 'model_id' => $model->getKey()]) }}">
                    </div>
                    <div class="col-md-3 d-flex justify-content-end">
                        <button type='button' data-confirm='Are you Sure ?'
                            data-action="{{ route('admin.media.remove_image', ['image_relation' => $image, 'model' => $model::class, 'model_id' => $model->getKey()]) }}"
