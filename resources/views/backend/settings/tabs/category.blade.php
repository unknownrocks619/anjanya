<div class="row">
    <form action="{{ route('admin.settings.page-setting') }}" method="post" class="ajax-form">
        ...
        <input type="hidden" name="setting_id" value="{{ $category_page->getKey() }}" />
        <div class="col-md-12">
                ...
                <tbody>
                    @foreach (\App\Models\ComponentBuilder::COMPONENTS_WIDGETS as $position_key => $positions)
                        <tr>
                            ...
                            <td>
                                @if (in_array($position_key, $category_page->additional_text['blocks']))
                            ...
                            <td>
                                <select name="position[]" class="form-control no-select-2">
                                    @for ($i = 0; $i < count(\App\Models\ComponentBuilder::COMPONENTS_WIDGETS); $i++)
                                        <option value="{{ $i }}"
                                            @if (
                                                $category_page->additional_text['blocks'] &&
                                                    isset($category_page->additional_text['blocks'][$i]) &&
                                                    $category_page->additional_text['blocks'][$i] == $position_key) selected @endif>{{ $i + 1 }}
                                        </option>
                                    @endfor
                                </select>
                            </td>
                            <td>
                                <select name="status[]" class="form-control no-select-2">
                                    <option value="1" @if (in_array($position_key, $category_page->additional_text['blocks'])) selected @endif>
                                        Enable
                                    </option>
                                    <option value="0" @if (!in_array($position_key, $category_page->additional_text['blocks'])) selected @endif>
                                        Disable
                                    </option>
                                </select>
                            </td>
